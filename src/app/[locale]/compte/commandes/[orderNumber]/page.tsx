...
export default async function AccountOrderPage({ params }: { params: Params }) {
  ...
  const order = await getAccountOrder(customer.id, orderNumber, locale);
  if (!order) notFound();
        ...
        <section className="mx-auto max-w-2xl px-6 py-12">
          ...
          <div className="mt-8 rounded-2xl border border-border bg-card p-6">
            ...
            <div className="mt-4 flex items-center justify-between border-t border-border pt-4">
              <span className="font-semibold text-deep">{t("total")}</span>
          ...
          <dl className="mt-6 grid gap-4 text-sm sm:grid-cols-2">
            <div className="rounded-2xl border border-border bg-card p-5">
              <dt className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">{t("delivery")}</dt>
              <dd className="mt-1 text-foreground">{order.location}</dd>
              <dd className="text-muted-foreground">{order.phone}</dd>
            </div>
            <div className="rounded-2xl border border-border bg-card p-5">
              <dt className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">{t("payment")}</dt>
